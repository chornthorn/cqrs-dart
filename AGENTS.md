...
- **Do not introduce deprecated aliases or typedefs**: Do not leave legacy aliases, fallback typedefs, or deprecated shims (e.g., no `typedef Singleton = Injectable;`, no legacy `@module`, no `@Named`).
...
- **Injectable Micro-Packages**:
  ...
  - Use `@thirdParty` for third-party provider modules and `@Inject('tag')` for qualifiers.
- **CQRS Micro-Packages**:
